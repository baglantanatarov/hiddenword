  ...
  void _submit() {
    ...
    if (_seen.contains(strict)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Бұл сөзді енгіздіңіз', style: TextStyle(color: Colors.white)),
        ...
        child: SafeArea(
          child: Column(
            children: [
              ...
              // PROGRESS BAR (fills 100% exactly)
// PROGRESS BAR (сол жақтан толады, 100% – толық)
Padding(
    ...
    child: ClipRRect(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          final double factor = (_lastPercent.clamp(0, 100) as num).toDouble() / 100.0;
          final double w = constraints.maxWidth * factor;
          return Align(
            alignment: Alignment.centerLeft, // ⬅️ Сол жақтан бастау
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 300),
              width: w,
              height: double.infinity,
              decoration: BoxDecoration(
                color: Color.lerp(Colors.red, Colors.green, _lastPercent / 100) ?? Colors.green,
                borderRadius: BorderRadius.circular(8),
              ),
            ),
          );
              ...
              if (_controller.text.isEmpty && _guesses.isEmpty)
                Expanded(
                  child: Center(
                    child: Opacity(
                      opacity: 0.7,
                      child: const Text('Болжамдағы кез келген сөздерді жаз', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white70)),
                    ),
                  ),
                )
              else
...
class _GlassIconButton extends StatefulWidget {
  ...
  const _GlassIconButton({required this.icon, this.tooltip, required this.onTap});
...
class _GlassIconButtonState extends State<_GlassIconButton> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    if ((widget.tooltip ?? '').isEmpty) return iconBtn;
    return Tooltip(message: widget.tooltip!, child: iconBtn);
  }
}
